...
class RealDataProcessor:
    ...
    def filter_real_fixtures(self, raw_data: Dict) -> List[Dict]:
        ...
        real_fixtures = [f for f in real_fixtures if f['id'] not in self.sample_fixture_ids]
            ...
            for fixture in data['data']:
                try:
                    ...
                    if len(participants) >= 2:
                        home_team = (
                            participants[0]
                            if (participants[0].get('meta', {}) or {}).get('location') == 'home'
                            else participants[1]
                        )
                        away_team = (
                            participants[1]
    ...
    def calculate_match_prediction(self, fixture: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            _ = (home_team_id, away_team_id)  # keep variables referenced
    ...
    def generate_betting_summary(self, fixtures: List[Dict], predictions: List[Dict], value_bets: List[Dict]) -> Dict:
        ...
        recommended_bets = len([bet for bet in value_bets if bet['recommendation'] in ['STRONG BUY', 'BUY', 'CONSIDER']])
...
# =========================
# INTEGRATION EXAMPLE (comments only; safe to copy into your bot)
# =========================
# def analyze_real_betting_opportunities(self):
#     # Get your raw data (the way you’re currently doing it)
#     raw_data = self.get_current_raw_data()  # Your existing method
#
#     # Process real betting opportunities
#     processor = RealDataProcessor(self.api)
#     betting_analysis = processor.process_real_betting_opportunities(raw_data)
#
#     # Log results
#     logger.info(f"Real betting analysis: {betting_analysis['summary']['summary_text']}")
#
#     # Save to database or display in dashboard
#     return betting_analysis
#
# # Call this instead of processing sample data:
# # real_opportunities = bot.analyze_real_betting_opportunities()